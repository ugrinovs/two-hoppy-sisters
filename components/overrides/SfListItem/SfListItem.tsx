...
const SfListItem = polymorphicForwardRef<
  typeof defaultListItemTag,
  SfListItemProps
>((props, ref) => {
  const {
  ...
  return (
    <Tag
      ...
      className={classNames(
        "inline-flex items-center gap-2 w-full cursor-pointer focus-visible:outline focus-visible:outline-offset focus-visible:relative focus-visible:z-10",
        {
    ...
    >
      {slotPrefix && (
        <ChildrenTag
          className={disabled ? "text-disabled-500" : "text-neutral-500"}
        >
          {slotPrefix}
        </ChildrenTag>
      )}
      <ChildrenTag className="flex flex-col w-full min-w-0">
        {children}
      </ChildrenTag>
      {slotSuffix && (
        <ChildrenTag
